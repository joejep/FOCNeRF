...
if __name__ == '__main__':
    ...
    parser.add_argument('--edit_object', type=str, default='bottle', help="object to edit")
    parser.add_argument('--offset_x', type=float, default=0.01, help="offset x")
    parser.add_argument('--offset_y', type=float, default=0.01, help="offset y")
    parser.add_argument('--offset_z', type=float, default=0.60, help="offset z")
    ...
    class MultiTrainer(Trainer):
        ...
        def compute_metrics_both_backgrounds(self, trainer, data, max_densities, max_rgbs, H, W, opt, i):
            ...
            self.frames["rgb_white"].append(np.asarray(white_rgb_img))
            self.frames["rgb_black"].append(np.asarray(black_rgb_img))
            ...
            # self.frames["gt_white"].append(np.asarray(white_gt_img))
            self.save_image(black_gt_img, gt_path_b)
            if len(self.frames["rgb_white"])>=10:
              
                # clip=ImageSequenceClip(self.frames["rgb_black"],fps=1)
                clip=ImageSequenceClip(self.frames["rgb_black"],fps=1)
                clip.write_videofile("black_bg_58.mp4",fps=1)
                clip.close()
        ...
        def modify_rays_for_object(self, rays_o, rays_d, yolo_details, ckpt):
            
            object_type = self.get_object_type_from_ckpt(ckpt)
            
            
            if object_type == opt.edit_object:
                offset_x = opt.offset_x  #0.13
                offset_y = opt.offset_y  #0.01
                offset_z = opt.offset_z
        
                new_rays_o = rays_o.clone()
                new_rays_d = rays_d.clone()
                
                
                new_rays_o[:, 0] += offset_x
                new_rays_o[:, 1] += offset_y
                new_rays_o[:, 2] += offset_z
            
                new_rays_d = F.normalize(new_rays_d, dim=-1)
                return new_rays_o, new_rays_d
            else:
                
                if not hasattr(self, 'first_rays_o') or not hasattr(self, 'first_rays_d'):
                   
                    self.first_rays_o = rays_o.clone()
                    self.first_rays_d = rays_d.clone()
                
               
                return self.first_rays_o, self.first_rays_d

        # def modify_rays_for_object_FOR_ALL_OBJEct_rotating(self, rays_o, rays_d, yolo_details, ckpt):
        #     # Extract the object type from the checkpoint path
        #     object_type = self.get_object_type_from_ckpt(ckpt)
        ...
        #     # Create new tensors for the modified rays
        #     new_rays_o = rays_o.clone()
        #     new_rays_d = rays_d.clone()
            
        #     # Apply the offsets to all ray origins
        #     new_rays_o[:, 0] += offset_x
        #     new_rays_o[:, 1] += offset_y
        #     new_rays_o[:, 2] += offset_z
        ...
        def get_object_type_from_ckpt(self, ckpt):
            
            object_types = ['book', 'chair', 'bottle', 'cup']
        ...
        def run(self, model, rays_o, rays_d, yolo_details, staged=True, max_ray_batch= 4096, **kwargs):
            ...
            z_vals = torch.linspace(0.0, 1.0, opt.num_steps, device=device).unsqueeze(0) 
            z_vals = z_vals.expand((N, opt.num_steps)) 
            
            z_vals = nears + (fars - nears) * z_vals
            sample_dist = (fars - nears) / opt.num_steps 
                ...
                for i, data in tqdm.tqdm(enumerate(loader), desc=f"Inference with {len(self.ckpt_list)} models"):    
                    ...
                    for ckpts in self.ckpt_list:
                        ...
                        with torch.cuda.amp.autocast(enabled=self.fp16):
                            ...
                            modified_rays_o, modified_rays_d = self.modify_rays_for_object(self.rays_o, self.rays_d, self.yolo_details, self.ckpt)
                            
                            results = self.batch_run(modified_rays_o, modified_rays_d, self.yolo_details, **vars(opt))
...
test_dataset = calculate_feats2_inference(test_dataset, obj_feats_paths)
